...
fn main() {
    ...
    let n: usize = env::args().nth(3).and_then(|v| v.parse().ok()).unwrap_or(4);
    let data = fs::read(&path).expect("read");
    let frames = mp4::extract_ac4_samples(&data).expect("mp4");
    for (i, p) in frames.iter().enumerate().take(n) {
        fs::write(format!("{out}/frame{i:03}.bin"), p).unwrap();
    }
    println!("wrote {} frames", n.min(frames.len()));
}
