#!/usr/bin/env python3
"""R494b: v10f — BAND-SPLIT pursuit: separate atom budgets for bass/mid.
...
Outputs: kw_stereo_v10g.wav, kw_montage_v10g.wav
"""
...
if __name__ == '__main__':
    ...
    pairs = load_pairs()
    shorts = load_shorts()
    backext = load_backext()
    # merge backext: long adds into inventory bodies, short adds into shorts
    for fr, added in backext.items():
        for b in added:
            if b['kind'] == 'L':
                inv.setdefault(fr, []).append(dict(s=b['s'], e=b['e'], w=b['w'],
                    m=b['m'], nz=b['nz'], c=b['c'], bc=b.get('bc', 0), ch='?'))
            else:
                shorts.setdefault(fr, []).append(dict(s=b['s'], e=b['e'],
                    tl=b['tl'], W=b['W'], nz=b['nz'], c=b['c'], off=b['off']))
    for fr in inv: inv[fr] = sorted(inv[fr], key=lambda b: b['s'])
    print(f'inventory: {len(inv)} frames; shorts: {len(shorts)}; backext: {len(backext)}; S pairs: {len(pairs)}')
    ...
    for fr, bodies in sorted(inv.items()):
        ...
        FN = Am.shape[0]
        fgrid = np.fft.rfftfreq(FN, 1 / 48000.0)
        mlo = (fgrid >= 700)          # mid/high mask
        def bsplit(x, hi):
            X = np.fft.rfft(x)
            X[~mlo if hi else mlo] = 0
            return np.fft.irfft(X, FN)
        Alo = np.stack([bsplit(Am[:, j], False) for j in range(Am.shape[1])], 1)
        Ahi = np.stack([bsplit(Am[:, j], True) for j in range(Am.shape[1])], 1)
        for M in (Alo, Ahi):
            M /= (np.linalg.norm(M, axis=0, keepdims=True) + 1e-12)
        def pursue_band(tb, Ab, K):
            resid = tb.copy(); sel = []
            for _ in range(K):
                dots = Ab.T @ resid
                ...
                sel.append(j)
                S = Ab[:, sel]
                wsel, *_ = np.linalg.lstsq(S, tb, rcond=None)
                resid = tb - S @ wsel
            if not sel:
                return np.zeros(FN)
            S = Ab[:, sel]
        ...
        def pursue(target, K=18):
            lo = pursue_band(bsplit(target.astype(float), False), Alo, 10)
            hi = pursue_band(bsplit(target.astype(float), True), Ahi, 26)
            return lo + hi
        tL = pursue(Rf[:, 0])
        tR = pursue(Rf[:, 1])
        spL_raw = fwd_mdct(tL)
    ...
    wv = wave.open('kw_stereo_v10g.wav', 'wb')
    ...
    if segs:
        ...
        wv = wave.open('kw_montage_v10g.wav', 'wb')
        wv.setnchannels(2); wv.setsampwidth(2); wv.setframerate(48000)
