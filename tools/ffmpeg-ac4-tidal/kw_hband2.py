...
for ch in range(2):
    ...
    for i in range(nf):
        ...
        for a,b in zip(edges[:-1],edges[1:]):
            ...
            # target = ref band shape scaled into vo's lowband energy
            re=np.mean(Rm[a:b]**2)/rlowE*vlowE
            g=np.sqrt(re/xe)
            X[a:b]*=np.clip(g,0.5,4.0)     # boost quiet mids, bounded
        # (2) SBR highband
        src=X[s0:xbin].copy()
...
wv=wave.open('kw_v13.wav','wb'); wv.setnchannels(2); wv.setsampwidth(2); wv.setframerate(SR)
wv.writeframes(out.astype(np.int16).tobytes()); wv.close()
print('wrote kw_v13.wav')
def bands(x):
