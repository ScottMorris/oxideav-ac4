...
static int check_sequence(AC4DecodeContext *s)
{
    ...
    av_log(s->avctx, AV_LOG_DEBUG, "unexpected sequence counter: %d vs %d\n", s->sequence_counter, s->sequence_counter_prev);
    return 0;
}
                                    ...
                                    SubstreamGroupInfo *g)
{
    ...
    if (g->channel_coded) {
        ...
        av_log(s->avctx, AV_LOG_WARNING, "object/A-JOC substream info not implemented, skipping frame\n");
        return AVERROR_INVALIDDATA;
      /*for (int sus = 0; sus < n_lf_substreams; sus++) {
            if (get_bits1(gb)) {
...
{
    if (getenv("AC4_MSFB5") && transf_length >= 384)
        return 5;
...
static int asf_section_data(AC4DecodeContext *s, Substream *ss, SubstreamChannel *ssch)
{
    ...
    for (int g = 0; g < ssch->scp.num_window_groups; g++) {
        ...
        if (gidx <= 2 || (getenv("AC4_W3_LONG") && ssch->scp.long_frame)) {
            sect_esc_val = (1 << 3) - 1;
            n_sect_bits = 3;
        } else {
            sect_esc_val = (1 << 5) - 1;
...
static int channel_element_7x(AC4DecodeContext *s, int channel_mode, int iframe)
{
    ...
    if (ss->codec_mode == CM_SIMPLE || ss->codec_mode == CM_ASPX) {
        if (get_bits1(gb)) { /* b_use_sap_add_ch */
            ret = chparam_info(s, ss, &ss->ssch[5]);
            if (ret < 0)
                return ret;
            ret = chparam_info(s, ss, &ss->ssch[6]);
            if (ret < 0)
                return ret;
        }
        ret = two_channel_data(s, ss, &ss->ssch[5], &ss->ssch[6], 2);
        if (ret < 0)
            return ret;
    }
    if (ss->codec_mode == CM_ASPX_ACPL_1) {
        ss->max_sfb_master = get_bits(gb, 5); // n_side_bits for largest TL
        ret = chparam_info(s, ss, &ss->ssch[5]);
        if (ret < 0)
            return ret;
        ret = chparam_info(s, ss, &ss->ssch[6]);
        if (ret < 0)
            return ret;
        ret = sf_data(s, ss, &ss->ssch[5], iframe, SF_ASF);
        if (ret < 0)
            return ret;
        ret = sf_data(s, ss, &ss->ssch[6], iframe, SF_ASF);
        if (ret < 0)
            return ret;
    }
