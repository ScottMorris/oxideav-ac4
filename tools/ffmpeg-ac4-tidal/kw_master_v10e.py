#!/usr/bin/env python3
"""R494: v10e — v10 + PRE-EMPHASIZED matching pursuit (whitened fit).
...
Outputs: kw_stereo_v10e.wav, kw_montage_v10e.wav
"""
...
if __name__ == '__main__':
    ...
    for fr, bodies in sorted(inv.items()):
        ...
        def preemph(x):
            ...
            y[1:] -= 0.95 * x[:-1]
            return y
        Ae = np.stack([preemph(Am[:, j]) for j in range(Am.shape[1])], 1)
        Ae /= (np.linalg.norm(Ae, axis=0, keepdims=True) + 1e-12)
        def pursue(target, K=18):
            ...
            for _ in range(K):
                ...
                sel.append(j)
                S = Ae[:, sel]
                wsel, *_ = np.linalg.lstsq(S, te, rcond=None)
                resid = te - S @ wsel
            if not sel:
                return np.zeros(len(target))
            # fit weights in emphasized domain, reconstruct with raw atoms
            S = Ae[:, sel]
            wsel, *_ = np.linalg.lstsq(S, te, rcond=None)
            return Am[:, sel] @ wsel
        tL = pursue(Rf[:, 0])
    ...
    wv = wave.open('kw_stereo_v10e.wav', 'wb')
    ...
    if segs:
        ...
        wv = wave.open('kw_montage_v10e.wav', 'wb')
        wv.setnchannels(2); wv.setsampwidth(2); wv.setframerate(48000)
