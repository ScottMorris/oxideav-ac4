...
Outputs: kw_stereo_v10f.wav, kw_montage_v10f.wav
"""
...
if __name__ == '__main__':
    ...
    for fr, bodies in sorted(inv.items()):
        ...
        def pursue(target, K=18):
            lo = pursue_band(bsplit(target.astype(float), False), Alo, 8)
            hi = pursue_band(bsplit(target.astype(float), True), Ahi, 14)
            return lo + hi
        tL = pursue(Rf[:, 0])
        tR = pursue(Rf[:, 1])
        spL_raw = fwd_mdct(tL)
    ...
    wv = wave.open('kw_stereo_v10f.wav', 'wb')
    ...
    if segs:
        ...
        wv = wave.open('kw_montage_v10f.wav', 'wb')
        wv.setnchannels(2); wv.setsampwidth(2); wv.setframerate(48000)
