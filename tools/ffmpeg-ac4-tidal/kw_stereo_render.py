...
sp=np.fromfile('/tmp/spec_ims2.bin',dtype=np.float32).astype(np.float64); nf=sp.size//(8*N); sp=sp[:nf*8*N].reshape(nf,8,N)
...
wv=wave.open('kw_stereo.wav','wb'); wv.setnchannels(2); wv.setsampwidth(2); wv.setframerate(48000)
wv.writeframes(oo.astype(np.int16).tobytes()); wv.close()
print(f'kw_stereo.wav peak={np.abs(oo).max():.0f}')
