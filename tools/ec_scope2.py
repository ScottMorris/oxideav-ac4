...
for qtab in ('30','15'):
    for noisen in (1,2,3):
        for ahlen in (3,6):
            for ticlen in (16,17,18,19,20):
                x1set={}
                for f0 in range(16):
                    for f1 in range(16):
                        P=dict(base, qtab=qtab, noisen=noisen, ahlen=ahlen, ticlen=ticlen, fres0=f0, fres1=f1)
                ...
                for x1,(f0a,f1a) in x1set.items():
                    for f0 in range(16):
                        for f1 in range(16):
                            ...
                            if x2==TARGET:
                                hits+=1
                                print("HIT q=%s noisen=%d ah=%d tic=%d | t1 fres(%x,%x)->%d | t2 fres(%x,%x)" % (qtab,noisen,ahlen,ticlen,f0a,f1a,x1,f0,f1))
print("done", hits)
